...
class StrategyPerformanceMonitor:
    ...
    def __init__(self):
        self.performance_dir = Path(__file__).parent.parent / "model" / "metrics"
...
class SmartStrategySelector:
    ...
    def __init__(self):
        ...
        self.strategies = {
            "snownlp": SnowNLPStrategy(),
            "llm": LLMStrategy(),
            "custom_model": CustomModelStrategy()
        }
    ...
    def analyze_with_strategy(self, text: str, strategy_name: str) -> SentimentResult:
        """使用指定策略进行分析
        ...
        try:
            if strategy_name in self.strategies:
                result = self.strategies[strategy_name].analyze(text)
            else:
                # 默认使用SnowNLP
                result = self.strategies["snownlp"].analyze(text)
                strategy_name = "snownlp"
        except Exception as e:
    ...
    def _estimate_accuracy(self, result: SentimentResult, text: str) -> float:
        """估计分析准确率
